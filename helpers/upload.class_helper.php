...
class UploadHandler
{
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index) {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file->size = $file_size;
			$f_name = $file->name;
			//add_img로 변수 보내기(db 저장용)
			$file->upload_to_db = $this->add_img($file_path,$type,$f_name,$name);
            $this->set_file_delete_url($file);
        }
        return $file;
    }
	...
	function add_img($f_path,$f_type,$name,$o_name){
		$furl = '/uploads/'.$name;
		//DB에 저장하기
		//$user=$this->session->userdata('ej_user');
		$add_to_db = $this->query("INSERT INTO upload_data (
		file_path,
		file_url,
		type,
		file_name,
		...
		VALUES ('".$f_path."','".$furl."','".$f_type."','".$name."','".$o_name."')") or die(mysql_error());
		return $add_to_db;
	}
