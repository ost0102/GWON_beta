<!--사용자가 참여한 정보-->
<? if(isset($act_project)){
		echo '
		<h3>연결된 프로젝트</h3>
		<hr style="margin-top:5px; margin-bottom: 10px;"/>';

		foreach ($act_project as $act_project)
		{
			//print_r($row);
			//class_no가 없을 경우 최근 값을 가져와라
			$cate_id = $act_project['cate_id'];
			$title = $act_project['title'];
			$summary = $act_project['summary'];
			$logo = $act_project['logo'];
			$date = $act_project['date'];
			$domain = $act_project['domain'];
			$p_num = $act_project['p_num'];
			$state = $act_project['state'];
			$project_img = $act_project['project_img'];
			if($state=='0'){
			...
			<div style='width: 100%; float:left; margin-top: 10px; margin-bottom: 10px;'>
				<table width='100%'>
					<tr>
						...
						<div class='circular' style='background:url(".$project_img.") #cdcdcd no-repeat center center; width: 90px; height:90px; background-size:100px 100px;'>
						</td>
						<td valign='top'>
